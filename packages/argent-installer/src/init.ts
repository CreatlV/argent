...
import {
  ...
  getMcpEntry,
  copyRulesAndAgents,
  summarizeRulesAndAgentsCopyForTelemetry,
...
const INSTALL_SKILLS_NPX_FAILED: InstallerFailureSignal = {
  error_code: FAILURE_CODES.INSTALL_SKILLS_NPX_FAILED,
  failure_stage: "installer_skills_npx",
  failure_area: "installer",
  error_kind: "subprocess",
};
...
const INSTALL_RULES_AGENTS_COPY_FAILED: InstallerFailureSignal = {
  ...
  failure_stage: "installer_rules_agents_copy",
  failure_area: "installer",
  error_kind: "subprocess",
};
...
export async function init(args: string[]): Promise<void> {
  ...
  if (skillsMethod === "manual") {
    ...
    try {
      ...
      await track("installation:skill_install_result", { is_success: true });
    } catch (err) {
      if (skillsMethod === "default") {
      ...
      await track("installation:skill_install_result", {
        is_success: false,
        ...INSTALL_SKILLS_NPX_FAILED,
      });
    }
  }
  ...
  const rulesAgentsCopySummary = summarizeRulesAndAgentsCopyForTelemetry(copyResults);
  await track("installation:rules_agents_copy", {
    ...rulesAgentsCopySummary,
    ...(rulesAgentsCopySummary.failed_count > 0 ? INSTALL_RULES_AGENTS_COPY_FAILED : {}),
  });
